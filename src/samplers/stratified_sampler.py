...
class StratifiedSampler(BaseSampler):
    ...
    def __init__(self, dimension: int, strata_per_dim: int = 5, device: str = "cpu"):
        ...
        self.strata_weights = torch.ones(strata_per_dim**dimension) / (
            strata_per_dim**dimension
        )
    ...
    def sample(self, n_samples: int) -> torch.Tensor:
        ...
        # Create grid points
        grid = torch.meshgrid(
            *[torch.linspace(0, 1, self.strata_per_dim) for _ in range(self.dimension)],
            indexing="ij",
        )
    ...
    def plot_strata(self, dims: Optional[Tuple[int, int]] = None) -> Figure:
        ...
        for d in range(1, 2):  # Only for the two plotted dimensions
            flat_indices = (
                flat_indices * self.strata_per_dim + strata_indices[:, dims[d]]
            )
