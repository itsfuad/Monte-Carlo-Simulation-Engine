...
class AdaptiveOptimizer(BaseOptimizer):
    ...
    def set_state(self, state: Dict[str, Any]) -> None:
        ...
        self.velocity = torch.tensor(state['velocity']) if state['velocity'] is not None else None
        self.best_value = state['best_value']
        self.best_params = torch.tensor(state['best_params']) if state['best_params'] is not None else None 
