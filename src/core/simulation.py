...
class MonteCarloSimulation:
    ...
    def _process_batch(
        self, batch_samples: torch.Tensor
    ...
    def _update_running_stats(
        self, batch_values: torch.Tensor, i: int, running_mean: Optional[torch.Tensor]
    ) -> torch.Tensor:
    ...
    def _optimize_and_update(
        ...
        opt_result = (
            self.optimizer.optimize(batch_samples, batch_values)
            if self.optimizer
            else {}
        )
    ...
    def run(self) -> Dict[str, Any]:
        ...
        with tqdm(total=n_batches, desc="Simulation Progress") as pbar:
            for i in range(n_batches):
                ...
                # Track best results
                batch_mean = torch.mean(batch_values)
                if best_mean is None or abs(batch_mean - np.pi / 4) < abs(
                    best_mean - np.pi / 4
                ):
                ...
                if self.optimizer is not None:
                    opt_result = self._optimize_and_update(
                        batch_samples, batch_values, best_samples, best_values
                    )
    ...
    def _finalize_results(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        final_results = {
            ...
            "values": results[
                "values"
            ],  # Keep original list of batches for pi estimation
