...
class SmartSampler(BaseSampler):
    ...
    def __init__(
        ...
        self.distributions = [
            Normal(mean, std) for mean, std in zip(self.means, self.stds)
        ]
    ...
    def update(self, samples: torch.Tensor, weights: torch.Tensor) -> None:
        ...
        for dim in range(self.dimension):
            ...
            self.means[dim] = (1 - self.learning_rate) * self.means[
                dim
            ] + self.learning_rate * weighted_mean
            self.stds[dim] = (1 - self.learning_rate) * self.stds[
                dim
    ...
    def plot_sampling_distribution(
        ...
        x = np.linspace(
            means[dims[0]] - 3 * stds[dims[0]], means[dims[0]] + 3 * stds[dims[0]], 100
        )
        y = np.linspace(
            means[dims[1]] - 3 * stds[dims[1]], means[dims[1]] + 3 * stds[dims[1]], 100
        )
        X, Y = np.meshgrid(x, y)
